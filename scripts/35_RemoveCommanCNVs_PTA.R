...
merged_read= merged_read[(merged_read$version%in%'LiftOver - T2T to hg38') & (merged_read$data.x%in%'PTA') , ]
################# ################# ################# ################# ################# #################
delimx= ".bowtie|.bt2"
...
################# ################# ################# ################# ################# #################
result4a= result4[(result4$version%in%'LiftOver - T2T to hg38') & (result4$data%in%'PTA') , ]
...
result4x= result3[result3$data%in%'PTA',]
...
saveRDS(ordered_datax, file = './data/processed/rds/CNVs_PTA_forcommonality.rds')
ordered_datax= ordered_datax[ordered_datax$cn_binsize>5,]
ordered_datax$cn_median = round(ordered_datax$cn_median, 2)
saveRDS(ordered_datax, file = './data/processed/rds/CNVs_PTA.rds')
...
saveRDS(ordered_datax_s, file = './data/processed/rds/SignificantCNVs_PTA.rds')

ordered_datax1= ordered_datax[,-which(colnames(ordered_datax)%in%c('generic_name_standardized', 'data.y','shared_id', 
                                                                   'generic_name' ,  'Run',"Bp",
...
colnames(signix3) = c("samples","shared_ind_number", "chr", "cn", "cn_median", "start", "end", "width", "id","ID")
...
for (i in cell_names) {
  ...
  xx4 <- xx3 %>% mutate(ID =      replace(ID, which(duplicated(ID)), ""))
