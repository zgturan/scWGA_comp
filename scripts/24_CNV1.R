...
all2x$data= sapply(qq, function(x)x[[1]])
all2x$data= gsub('Pico','PicoPLEX', all2x$data)
all2x[,ncol(all2x) + 1]= sapply(qq, function(x)x[[4]])
colnames(all2x)[ncol(all2x)]='version'
...
saveRDS(all2x_hg38, file= './data/processed/rds/Confid_MAD_VariableSize_hg38_xlsx_CNV1.rds')
saveRDS(all2x_lift, file= './data/processed/rds/Confid_MAD_VariableSize_liftover_xlsx_CNV1.rds')
saveRDS(all2x_t2t, file= './data/processed/rds/Confid_MAD_VariableSize_t2t_xlsx_CNV1.rds')