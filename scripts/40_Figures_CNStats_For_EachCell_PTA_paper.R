...
for (i in 1:length(aa2)){
  ...
  if (all(variablesx == variablesx[1])){
    ...
    if(file.exists(cell_stat_path)){
      ...
      if (filenamex == "PTA_48_500kb_lift") {
        filenamey <- "PTA_48bp_500kb_Liftover"
      } else {
        filenamey <- "PTA_101bp_500kb_Liftover"
      }
      ...
      ggsave(first, filename = file.path('./results/CN_plotsx/PTA_paper/pdf',paste0(aa[i],'.pdf')),
             unit="cm", width = 130, height = 50, useDingbats = F, limitsize = FALSE)
